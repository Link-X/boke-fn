export const setBtnAnt = (el, data = { }) => {
  ...
  const show = function (event) {
    ...
    const rectObj = forRect(pDiv),
      ...
      _scale = 'scale(' + pDiv.clientWidth / 130 * 10 + ')'
      const position = {
      top: _height + 'px',
      left: _left + 'px'
    }
  ...
  el.addEventListener('click', function (e) {
        if (base.type === 'agent' && base.typeNodeName && e.target.nodeName === base.typeNodeName.toLocaleUpperCase()) {
          show(e)
        } else {
          show(e)
        }
